...
public class CafeteriaChairFront : MonoBehaviour
{
    private static CafeteriaChairFront currentChair;
    ...
    void Start()
    {
        ...
        if (outlineObject == null)
            Debug.LogError("[CafeteriaChairFront] outlineObject가 설정되지 않았습니다!");

        if (qKeyImage == null)
            Debug.LogError("[CafeteriaChairFront] qKeyImage가 설정되지 않았습니다!");
    ...
    {
        Debug.Log("[CafeteriaChairFront] OnTriggerEnter2D 실행됨: " + other.name);
    ...
    {
        var allChairs = FindObjectsOfType<CafeteriaChairFront>();
